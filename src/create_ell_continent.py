...
def work_with_df(df: pd.DataFrame):
    ...
    df.loc[:, "POL SHORT"] = df["POL_CONTINENT"].str[:2]
...
def cargo_detail(df: pd.DataFrame):
    ...
    df_cd.loc[:, "POL"] = df["POL_CONTINENT"]
    df_cd.loc[:, "Pol terminal"] = df["TOL_CONTINENT"]
...
def manifest(df: pd.DataFrame):
    ...
    df_man.loc[:, "Pod Terminal"] = df["TOL_CONTINENT"]
